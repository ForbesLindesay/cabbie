    ...
    visitor: {
      VariableDeclaration(path) {
        ...
        if (
          ...
          !/Enum$/.test(declaration.node.id.name)
        ) {
          return;
        }
        ...
        init.node.properties.forEach((p, i) => {
          ...
          if (!t.isProperty(property.node)) {
            throw property.buildCodeFrameError(
              'Enums values must be ordinary properties.',
              SyntaxError,
            );
          }
          ...
          if (
            ...
            !t.isStringLiteral(value.node)
          ) {
            throw value.buildCodeFrameError(
              'Enums values must be numberic literals or string literals.',
              SyntaxError,
            );
          }
          const typeAnnotation = t.isNumericLiteral(value.node)
          ...
          value.replaceWith(
            t.typeCastExpression(value.node, t.typeAnnotation(typeAnnotation)),
          );
        });
