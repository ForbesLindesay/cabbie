...
Connection.prototype.sessionRequest = function (session, method, path, body) {
  ...
  function makeRequest (localSession) {
    ...
    return self.request(method, path[0] === '/' || path === '' ? undefined : path, path[0] === '/' || path === '' ? '/session/' + localSession.getId() + path : undefined, rawBody, { 'Content-Type': 'application/json;charset=UTF-8' });
  }
