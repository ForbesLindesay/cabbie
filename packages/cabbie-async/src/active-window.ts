...
class ActiveWindow extends BaseWindow {
  ...
  async execute(
    ...
    return this.driver.requestJSON('POST', '/execute', {
      script: codeToString(script),
      args,
    });
  }
  ...
  async asyncExecute(
    ...
    await this.driver.requestJSON('POST', '/execute_async', {
      script: codeToString(script),
      args,
    });
  }
  ...
  async sendKeys(str: string | Array<string>): Promise<void> {
    await this.driver.requestJSON('POST', '/keys', {
      value: Array.isArray(str) ? str : [str],
    });
  }
  ...
  async getActiveElement(): Promise<Element> {
    const elementHandle = await this.driver.requestJSON(
      'POST',
      '/element/active',
    );
  ...
  async getElement(
    ...
    const elementHandle = await waitFor(() =>
      this.driver.requestJSON('POST', '/element', {
        using: selectorType,
        value: selector,
      }),
    );
  ...
  async getElementsByTextContent(
    ...
    if (
      selector === 'a' &&
      (selectorType === SelectorType.CSS || selectorType === SelectorType.TAG)
    ) {
  ...
  async getElementByTextContent(
    ...
    const element = await waitFor(() =>
      this.tryGetElementByTextContent(textContent, selector, selectorType),
    );
    if (element) {
      return element;
    }
    const err = new Error(
      'Could not find an element with the text content: ' + textContent,
    );
