...
class Driver {
  ...
  _createSessionFromExistingID(seesionId: string, capabilities: Object): Session {
    return new Session({ sessionId, capabilities });
  }
  _createSession(desiredCapabilities: Object, requiredCapabilities?: Object): Session {
    ...
    const res = this._connection.request('POST', '/session', {
      json: capabilities
    });
  ...
  getApplicationCacheStatus(): ApplicationCacheStatus {
    return this.requestJSON('GET', '/application_cache/status');
  }
  ...
  getLogs(logType: string): Array<LogEntry> {
    const logs = this.requestJSON('POST', '/log', {
      type: logType
    });
  ...
  getLogTypes(): Array<string> {
    return this.requestJSON('GET', '/log/types');
  }

  /**
   * End this Driver session
   */
  dispose(status?: Object): void {
    if (status) {
      this.jobUpdate(status);
    }
  ...
  sauceJobUpdate(body: Object): Boolean {
    ...
    request('PUT', 'http://' + auth + '@saucelabs.com/rest/v1/' + auth.split(':')[0] + '/jobs/' + session.id(), {
      json: body
    });
 ...
 * @return {Status}
 */
Driver.getStatus = function (remote: string, mode) {
  var request = new Connection(remote, mode);

  const response = when(request.request('GET', '/status'), Connection._parseResponse);
...
Driver.getSessions = function (remote, mode) {
  ...
  const rawSessions = request.request('GET', '/sessions');
 ...
 * Setup debug output
 */
function setupDebug(driver: Driver, options: object) {
  var indentation = 0;
