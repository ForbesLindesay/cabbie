...
async function run(driver: Driver, location: string) {
  ...
  await test('test timeouts', async () => {
    await driver.timeOut.setTimeOuts({
      'implicit': '1s',
      'async': '10s',
    });
  });
  ...
  await test('submit a form', async () => {
    ...
    assert.equal(
      (await driver.browser.activeWindow.navigator.getUrl()).substr(-7),
      '?q=1357',
    );
  });
  ...
  await test('click on an element', async () => {
    ...
    assert.equal(
      await areaToClick.getText(),
      'clicked left at 450x75',
    );
  });
  ...
  await test('get the title of the active window', async () => {
    assert.equal(
      await driver.browser.activeWindow.getTitle(),
      'Test Page',
    );
  });
  ...
  await test("accept an alert", async () => {
    ...
    assert.equal(
      await driver.browser.activeWindow.alert.getText(),
      'This is a test alert!',
    );
    await driver.browser.activeWindow.alert.accept();
    assert.equal(
      await alertButton.getText(),
      'alerted'
    );
  });
  ...
  await test("accept a prompt with default value", async () => {
    ...
    await driver.browser.activeWindow.alert.accept();
    assert.equal(
      await promptButton.getText(),
      'prompted: default value',
    );
  });
  ...
  await test('execute javascript code as a function', async () => {
    await driver.browser.activeWindow.execute(function () { alert("test-33"); });
    assert.equal(
      await driver.browser.activeWindow.alert.getText(),
      'test-33',
    );
  ...
  await test('execute javascript code as a function with parameters', async () => {
    const alertButtonText = await driver.browser.activeWindow.execute(function (id) {
  ...
  await test('clear the cookie-storage', async () => {
    ...
    assert.equal(
      await driver.browser.cookieStorage.getSize(),
      0,
    );
  });
  ...
  await test('get a value in local-storage', async () => {
    assert.equal(
      await driver.browser.localStorage.getItem('testKey'),
      '2468',
    );
  });
  ...
  await test('get the size of local-storage', async () => {
    assert.equal(
      await driver.browser.localStorage.getSize(),
      2,
    );
  });
  ...
  await test('get a value in session-storage', async () => {
    assert.equal(
      await driver.browser.sessionStorage.getItem('testKey'),
      '2468',
    );
  });
  ...
  await test('get the size of session-storage', async () => {
    assert.equal(
      await driver.browser.sessionStorage.getSize(),
      2,
    );
  });
